...
// Identidad del Jugador y Autenticación
let displayUsername = "Jugador_" + Math.floor(Math.random() * 9999);
...
// ==========================================
// 2. ESTADO GLOBAL COMPARTIDO (Con Recuperación por Refresh)
// ==========================================
let sharedState = {
...
// Recuperar estado del host si recargó la página por error
let savedHostState = sessionStorage.getItem('bj_host_state');
if (savedHostState) {
...
// ==========================================
let secretHostDeck = []; 
let savedHostDeck = sessionStorage.getItem('bj_host_deck');
if (savedHostDeck) {
...
function generateSecureDeck() {
    ...
    sessionStorage.setItem('bj_host_deck', JSON.stringify(fullDeck));
    return fullDeck;
}

// ESTADO LOCAL DEL CLIENTE (Recuperable tras F5)
let balance = 5000;
let currentBets = JSON.parse(sessionStorage.getItem('bj_current_bets') || '{"main":0,"21+3":0,"pairs":0}');
let lastBets = JSON.parse(sessionStorage.getItem('bj_last_bets') || '{"main":0,"21+3":0,"pairs":0}');
let selectedChipValue = 100;
let imPlaying = sessionStorage.getItem('bj_im_playing') === 'true'; 
...
async function cargarPerfil() {
    try {
        ...
        if (session && session.user) {
            ...
            if (data) {
                balance = data.saldo != null ? data.saldo : 5000;
                displayUsername = data.username || displayUsername;
            }
        } else {
            let savedBalance = localStorage.getItem('bj_balance');
            balance = savedBalance ? parseInt(savedBalance) : 5000;
            let savedName = localStorage.getItem('bj_username');
            if (savedName) displayUsername = savedName;
            else localStorage.setItem('bj_username', displayUsername);
        }
    } catch(e) {
...
async function modificarSaldo(monto) {
    ...
    if (usuarioAutenticado) {
        ...
        } catch(e) { console.log("Error guardando saldo en DB", e); }
    } else {
...
function gameLoop() {
    ...
    if (sharedState.gameState === 'BETTING') roundResolved = false;
...
function emitState() {
    if (isHost) {
        sessionStorage.setItem('bj_host_state', JSON.stringify(sharedState));
        sessionStorage.setItem('bj_host_deck', JSON.stringify(secretHostDeck));
    }
...
function addBet(type) {
    ...
    if (balance >= selectedChipValue) {
        ...
        sessionStorage.setItem('bj_current_bets', JSON.stringify(currentBets));
        renderGameUI(); 
...
function confirmBet() {
    ...
    imPlaying = true;
    sessionStorage.setItem('bj_im_playing', 'true');
    lastBets = { ...currentBets };
    sessionStorage.setItem('bj_last_bets', JSON.stringify(lastBets));
...
function evaluarPagosLocales() {
    if (sharedState.gameState === 'RESOLVED' && !roundResolved) {
        ...
        imPlaying = false;
        sessionStorage.setItem('bj_im_playing', 'false');
        ...
        setTimeout(() => { 
            if (isHost) { 
                ...
                // LIMPIEZA TOTAL DE CARTAS AL INICIAR NUEVA RONDA
                sharedState.communityCards = []; 
                sharedState.dealerHand = []; 
                emitState(); 
            } 
